...
asa_dat <- read.csv('data/asa_equal_3.csv')
...
write.csv(asa_dat, 'metareg/asa_equal_3.csv')

# Weigh studies using inverse variance
asa_dat <- asa_dat[order(asa_dat$First.Author), ]
asa_meta <- rma(measure = 'OR', yi = logOR, sei = se, dat = asa_dat)

# Create forest plot and add results from tests of heterogeneity

forest(asa_meta, slab = asa_dat$First.Author, header = T, atransf = exp, xlab = 'Odds Ratio', main = 'ASA 3')
...
forest(asa_metafe, slab = asa_dat$Study, header = T, atransf = exp, xlab = 'Odds Ratio', main = 'ASA equal to 3 (FE model)')
